...
import { ClipboardCheck, FileText, MessageSquare, Building2, Globe, Target, Package, MapPin, Loader2, AlertCircle, File } from "lucide-react";
...
const OnboardingPanel = () => {
  ...
  const { data: onboardingData, isLoading, isError } = useOnboardingData();
      ...
      <Card className="bg-transparent p-0 border-none">
        <CardHeader className="p-0 mb-4">
          ...
          <p className="text-[10px] text-white/70">
            {hasData ? "Your onboarding information is displayed below. Click 'Talk to onboarding' to make changes." : "Follow these steps so we can activate your account and calibrate the right AI copilots for your team."}
          </p>
        </CardHeader>
        <CardContent className="space-y-4 p-0">
          {isLoading ? (
            ...
            <div className="rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-4">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[10px] text-red-400">{isError}</p>
              </div>
            </div>
          ) : hasData ? (
            <>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {onboardingSteps.map((step) => {
                ...
                return (
                  <div key={step.title} className="flex flex-col gap-2 rounded-2xl border border-white/10 bg-white/5 p-4">
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-[10px] text-white/70"> {step.description} </p>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-4">
            ...
            <Button onClick={handleTalkToOnboarding}
