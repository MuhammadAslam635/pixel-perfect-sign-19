...
import { useCreateFollowupTemplate, useUpdateFollowupTemplate, } from "@/hooks/useFollowupTemplates";
import { FollowupTemplate, FollowupTemplatePayload, } from "@/services/followupTemplates.service";
import { useToast } from "@/hooks/use-toast";
import { Mail, MessageSquare, Phone, Calendar as CalendarIcon, } from "lucide-react";
import { defaultFormValues, followupTemplateSchema } from "@/schema/followupTemplateSchema";
...
const TemplateFormModal = ({ open, onOpenChange, mode, template, }: TemplateFormModalProps) => {
  ...
  const { mutate: createTemplate, isPending: isCreating } = useCreateFollowupTemplate();
  const { mutate: updateTemplate, isPending: isUpdating } = useUpdateFollowupTemplate();
  ...
  const handleSubmitForm = (values: FollowupTemplateFormValues) => {
    ...
    if (mode === "edit" && template) {
      updateTemplate(
        ...
        {
          onSuccess: (response) => {
            toast({ title: "Template updated", description: response.message || "Followup template updated successfully.", });
            closeForm();
          },
          onError: (mutationError) => {
            toast({ title: "Unable to update template", description: getErrorMessage(mutationError, "Please try again."), variant: "destructive", });
          },
        }
      );
    } else {
      createTemplate(payload, {
        onSuccess: (response) => {
          toast({ title: "Template created", description: response.message || "Followup template created successfully.", });
          closeForm();
        },
        onError: (mutationError) => {
          toast({ title: "Unable to create template", description: getErrorMessage(mutationError, "Please try again."), variant: "destructive", });
        },
      });
    }
  };
        ...
        <div className="relative z-10 flex flex-col h-full min-h-0">
          <DialogHeader className="px-6 pt-6 pb-4 flex-shrink-0 border-b border-white/10">
            ...
            <DialogDescription className="text-xs text-white/70">
              {mode === "edit" ? "Update the followup template details" : "Create a new followup template"}
            </DialogDescription>
          </DialogHeader>
            ...
            <form onSubmit={form.handleSubmit(handleSubmitForm)} className="flex flex-col flex-1 min-h-0 overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto px-6 space-y-4 scrollbar-hide py-4 min-h-0">
                <div className="grid gap-5 md:grid-cols-2">
                  ...
                  <FormField control={form.control} name="numberOfDaysToRun" render={({ field }) => (
                    <FormItem className="space-y-2">
                      <FormLabel className={formLabelClasses}>Run Duration (Days)</FormLabel>
                      <FormControl>
                  ...
                  <FormField control={form.control} name="numberOfEmails" render={({ field }) => (
                    <FormItem className="space-y-2">
                      <FormLabel className={formLabelClasses}>Emails</FormLabel>
                      <FormControl>
                        <div className="relative">
                          <Input
                            {...field}
                  ...
                  <FormField control={form.control} name="numberOfCalls" render={({ field }) => (
                    <FormItem className="space-y-2">
                      <FormLabel className={formLabelClasses}>Calls</FormLabel>
                      <FormControl>
                        <div className="relative">
                          <Input
                            {...field}
                  ...
                  <FormField control={form.control} name="numberOfWhatsappMessages" render={({ field }) => (
                    <FormItem className="space-y-2">
                      <FormLabel className={formLabelClasses}>WhatsApp Messages</FormLabel>
                      <FormControl>
                        <div className="relative">
                          <Input
                            {...field}
              ...
              <DialogFooter className="px-6 py-4 flex-shrink-0 border-t border-white/10 gap-2">
                <Button type="button" variant="outline" onClick={closeForm} disabled={isCreating || isUpdating} className="bg-white/5 border-white/20 text-white hover:bg-white/10 hover:text-white transition-all text-xs">
                  Cancel
                </Button>
                <Button type="submit" disabled={isCreating || isUpdating} className="bg-primary hover:bg-primary/90 text-white transition-all text-xs">
                  {isCreating || isUpdating ? mode === "edit" ? "Updating..." : "Creating..." : mode === "edit" ? "Update Template" : "Create Template"}
                </Button>
              </DialogFooter>
            </form>
          </Form>
