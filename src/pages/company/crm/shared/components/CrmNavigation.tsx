...
export const CrmNavigation = () => {
  ...
  return (
    <nav className="hidden md:flex scrollbar-hide  min-w-0 w-full w-auto items-center justify-start gap-2 overflow-x-auto flex-nowrap snap-x snap-mandatory ">
      {visibleLinks.map((link) => {
        ...
        return (
          <button
            key={link.id}
            className={`group relative overflow-hidden flex-none flex h-10 items-center justify-center rounded-full border border-white/40 px-0 text-xs font-medium tracking-wide !transition-none sm:text-sm ${
              isActive
          ...
          >
            ...
            <Icon
              className={`h-[16px] w-[16px] flex-shrink-0 !transition-none ${
                isActive
            ...
            />
            <span
              className={`whitespace-nowrap !transition-none overflow-hidden ${
                isActive
