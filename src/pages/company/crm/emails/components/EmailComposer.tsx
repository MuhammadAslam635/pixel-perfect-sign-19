...
export const EmailComposer = ({ initialTo = [], initialSubject = "", initialBody = "", threadId, onSend, onCancel, isLoading = false, }: EmailComposerProps) => {
  ...
  const handleEnhanceWithAI = async () => {
    ...
    if (to.length === 0) {
      toast({ title: "No recipient", description: "Please add a recipient email address first.", variant: "destructive", });
      return;
    }
    ...
    try {
      ...
      if (!leadResponse.success || !leadResponse.data.lead) {
        toast({ title: "Lead not found", description: "The recipient must be a lead in your CRM to generate personalized emails using knowledge base context.", variant: "destructive", });
      ...
      if (!lead.companyId || !lead._id) {
        toast({ title: "Incomplete lead information", description: "Lead information is incomplete for generating suggestions.", variant: "destructive", });
        setIsEnhancing(false);
        return;
      }
