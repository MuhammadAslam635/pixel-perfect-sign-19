...
const EmailDetailPage = () => {
  ...
  const markReadMutation = useMutation({
    mutationFn: (isRead: boolean) => emailService.markEmailRead(emailId!, { isRead }),
    onSuccess: () => {
  ...
  const starMutation = useMutation({
    mutationFn: (isStarred: boolean) => emailService.starEmail(emailId!, { isStarred }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["email", emailId] });
      queryClient.invalidateQueries({ queryKey: ["inbox"] });
    },
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({ title: "Error", description: error.response?.data?.message || "Failed to delete email. Please try again.", variant: "destructive", });
    },
  });
  ...
  const handleReply = () => {
    ...
    navigate(`/emails/compose?to=${replyTo}&subject=${encodeURIComponent(email.subject)}&threadId=${email.threadId || ""}`);
  };
