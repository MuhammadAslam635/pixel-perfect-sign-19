...
export const CallView = ({
  ...
  const [selectedCallLog, setSelectedCallLog] = useState<LeadCallLog | null>(
    null
  );
  const [activeTab, setActiveTab] = useState<"recording" | "followup">(
    "recording"
  );
  ...
  const handleOpenCallDetails = useCallback(
    async (log: LeadCallLog) => {
      ...
      try {
        const refreshedResponse = await twilioService.getLeadCallLogs(
          leadId || "",
          {
            limit: 10,
          }
        ...
        refreshedLog =
          refreshedLogs.find((l: LeadCallLog) => l._id === log._id) || null;
        ...
        console.warn(
          "[handleOpenCallDetails] Failed to refresh call log, using cached data",
          refreshError
        );
      ...
      try {
        ...
        const recordingUrl =
          currentLog.elevenlabsRecordingUrl || currentLog.recordingUrl || null;
        ...
        const currentLog = refreshedLog || log;
        if (
          !currentLog.recordingSid &&
          !currentLog.elevenlabsRecordingUrl &&
          !currentLog.recordingUrl
        ) {
  ...
  useEffect(() => {
    ...
    if (
      (!selectedCallLogView || selectedCallLogView.type !== "recording") &&
      !selectedCallLog
    ) {
  ...
  useEffect(() => {
    ...
    const hasPendingAnalysis = callLogs.some((log) => {
      const startedAtMs = log.startedAt ? new Date(log.startedAt).getTime() : 0;
      const isRecent =
  ...
  return (
    <div className="grid grid-cols-2 gap-6 flex-1 w-full h-full px-0 pt-2 overflow-hidden">
      {/* Left Side: Call History - 1/2 width */}
      <div className="col-span-1 flex flex-col text-left space-y-3 overflow-y-auto scrollbar-hide">
                ...
                >
                  <div className="flex items-center justify-between px-4 py-3 gap-4">
                    ...
                    <div className="flex items-center gap-3 flex-1">
                      {/* Phone Icon */}
                      <div
                        className={`w-7 h-7 rounded-full ${iconBgColor} flex items-center justify-center flex-shrink-0`}
                      >
                        <svg
      ...
      {/* Right Side: Web Call / Call Details - 1/2 width */}
              ...
              <div className="flex flex-col gap-2 pb-4 border-b border-white/10">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex flex-col items-start">
                    ...
                    <span className="text-xs text-white/60">
                      {formatCallDate(selectedCallLog.startedAt)} •{" "}
                      {formatDuration(selectedCallLog.durationSeconds)}
                    </span>
                  </div>
                </div>
              </div>
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <span className="text-sm font-semibold text-cyan-400">
                                {selectedCallLog.leadSuccessScore}/100
                              </span>
                            </div>
                            <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${selectedCallLog.leadSuccessScore}%`,
                                }}
                              />
                            </div>
                          </div>
            ...
            <>
                  ...
                  <div className="flex items-center justify-center pt-5">
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{
                          background:
                            "linear-gradient(180deg, transparent 0%, rgba(34, 211, 238, 0.4) 50%, transparent 100%)",
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-xs  text-white/40">
                  Calls are routed via Twilio Voice with mic data processed
                  locally.
                </p>
              </div>
            </>
                      ...
                      recordingAudioUrl && (
                        <div className="space-y-4">
                          ...
                          <div className="flex items-start gap-2 px-2">
                            <svg
                              className="w-4 h-4 text-cyan-400/60 mt-0.5 flex-shrink-0"
                              fill="currentColor"
                              viewBox="0 0 20 20"
                            >
                              <path
