...
import { ArrowLeft, MessageSquare, Calendar, Mail, ChevronRight, AlertCircle, CheckCircle, XCircle, Timer } from "lucide-react";
...
const FeedbackItem = ({
    ...
    return (
        <div onClick={() => onClick(feedback._id)} className="group relative bg-black/20 border border-white/10 rounded-lg p-4 hover:border-cyan-400/50 hover:bg-black/30 transition-all duration-300 cursor-pointer">
            <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                        <Badge className={`${typeConfig.color} border flex items-center gap-1`}>
                    ...
                    {feedback.description && (
                        <p className="text-white/50 text-sm line-clamp-2 mb-2">{feedback.description}</p>
                    )}
...
const UserFeedbackList = () => {
    ...
    const fetchUserFeedbacks = async () => {
        ...
        try {
            const data = await feedbackService.getAllFeedbacks({ userId, page: 1, limit: 1000 });
            setFeedbacks(data.feedbacks || []);
        } catch (error) {
            setFeedbacks([]);
            toast.error(sanitizeErrorMessage(error, "Unable to load feedbacks. Please try again."));
        } finally {
            setLoading(false);
        }
    };
    ...
    const stats = useMemo(
        () => [
            { title: "Total Feedbacks", value: feedbacks.length, icon: MessageSquare, color: "text-white", iconClass: "text-white/70" },
            { title: "Open", value: feedbacks.filter((f) => f.status === "open").length, icon: AlertCircle, color: "text-yellow-400", iconClass: "text-white/70" },
            { title: "In Progress", value: feedbacks.filter((f) => f.status === "in-progress").length, icon: Timer, color: "text-blue-400", iconClass: "text-blue-400" },
            { title: "Closed", value: feedbacks.filter((f) => f.status === "closed").length, icon: CheckCircle, color: "text-green-400", iconClass: "text-white/70" },
        ],
        [feedbacks]
    );
    const handleFeedbackClick = (id: string) => navigate(`/admin/feedbacks/${id}`);
    if (userRoleName !== "Admin") {
        return (
            <AdminLayout>
        ...
        <AdminLayout>
            <main className="relative px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-[66px] mt-20 mb-8 flex flex-col gap-6 text-white flex-1 overflow-y-auto">
                ...
                <div className="flex flex-col gap-4 pt-4">
                    <Button variant="ghost" onClick={() => navigate("/admin/feedbacks")} className="w-fit text-white/70 hover:text-white hover:bg-white/10">
                        ...
                        {loading || !feedbacks.length ? (
                            ...
                            <div className="space-y-3">
                                {filteredFeedbacks.map((f) => (
                                    <FeedbackItem key={f._id} feedback={f} onClick={handleFeedbackClick} />
                                ))}
                            </div>
                        )}
                    </CardContent>
