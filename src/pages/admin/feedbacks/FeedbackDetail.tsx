...
const FeedbackDetail = () => {
  ...
  const canAccessAdminFeedback =
    userRoleName === "Admin" ||
    userRoleName === "CompanyAdmin" ||
    userRoleName === "Company";

  useEffect(() => {
    if (!canAccessAdminFeedback) {
        ...
        <div className="max-w-5xl mx-auto w-full space-y-6">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => navigate(`/feedback/${feedbackId}/chat`)}
          ...
          {/* Admin notes (visible to correspondent user) */}
          {feedback.adminNotes && feedback.adminNotes.length > 0 && (
            <Card className="bg-[linear-gradient(135deg,rgba(58,62,75,0.82),rgba(28,30,40,0.94))] border-white/10">
              <CardHeader className="pb-3">
                <CardTitle className="text-white flex items-center gap-2 text-lg">
                  <StickyNote className="w-5 h-5" />
                  Notes from support ({feedback.adminNotes.length})
                </CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <DialogContent className="sm:max-w-[500px] bg-[#0a0a0a] border-white/10 text-white">
            <DialogHeader>
              <DialogTitle className="text-xl font-bold flex items-center gap-2">
                ...
                Close Feedback Submission
              </DialogTitle>
            </DialogHeader>
            <div className="py-4 space-y-4">
              <p className="text-white/70 text-sm leading-relaxed">
                Add an optional closing note or resolution details that will be
                visible to the user.
              </p>
              <div className="space-y-2">
                <Textarea
                  placeholder="Details of the fix or why this is being closed..."
