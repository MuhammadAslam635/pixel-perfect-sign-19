...
export default function LeftChatPane({ leftCardRef, isMobile, leftPos, recording, setRecording, editMode, setEditMode }: Props) {
  const cardStyle = isMobile
    ? { position: 'fixed' as const, bottom: 12, left: '50%', transform: 'translateX(-50%)', width: 340 }
    : { position: 'fixed' as const, top: leftPos.top, left: leftPos.left, width: 560 };
    ...
    >
      ...
      <div className="absolute top-6 left-6">
        <div className="chat-tools">
          <Button size="icon" variant="ghost" className="chat-tool-btn" aria-label="List">
      ...
      <div className="relative z-10 text-center">
        <h1 className="text-[2.5rem] font-semibold mb-2 text-foreground leading-tight">Good Morning, Zubair!</h1>
      ...
      <div className="absolute bottom-4 left-4 right-4 md:bottom-6 md:left-6 md:right-6">
        <div className="chat-input-bar flex items-center gap-4 bg-[#2F2F2F]/70 backdrop-blur-sm border border-[#5A5A5A]/50">
          ...
          <Button
            ...
            className={`chat-icon-btn w-9 h-9 rounded-full flex-shrink-0 ${recording ? 'mic-recording' : 'bg-[#464646]/60 hover:bg-[#5A5A5A]/60'}`}
            aria-pressed={recording}
            aria-label={recording ? 'Stop recording' : 'Start recording'}
          >
            <Mic className="w-4 h-4 text-foreground/90" />
          </Button>
          <Button size="icon" className="chat-send-btn w-9 h-9 rounded-full bg-primary hover:bg-primary/90 flex-shrink-0">
